...
`background` and `ink` are a pair: the ambient ramp runs from one to the other,
so dim cells recede into the background rather than sitting on top of it. That
is what makes a light background work — set `background: '#f5f2ea'` and
`ink: '#8a8578'` for ASCII on paper.

Under `prefers-reduced-motion: reduce` the ambient animation freezes and the
pointer stays live, because a response to your own cursor is not autoplaying
motion. Opt out with `respectMotionPreference: false`.
